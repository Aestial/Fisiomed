...
namespace Fisiomed.Auth
{
	public class AuthHandler : MonoBehaviour 
	{
		...
		public Task SigninWithEmailAsync() {
			Debug.Log(String.Format("Attempting to sign in as {0}...", uc.myUser.email));
			// DisableUI();
			if (signInAndFetchProfile) {
			return auth.SignInAndRetrieveDataWithCredentialAsync(
				Firebase.Auth.EmailAuthProvider.GetCredential(uc.myUser.email, uc.myUser.password)).ContinueWith(
				HandleSignInWithSignInResult);
			} else {
			return auth.SignInWithEmailAndPasswordAsync(uc.myUser.email, uc.myUser.password)
